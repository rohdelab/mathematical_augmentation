...
import h5py
import sys
sys.path.append('../PyTransKit_local/')   # path to 'pytranskit' package

# Import RCDT-NS class from pytranskit package.
# This contains all the necessary functions to run the classifier
from pytranskit_local.classification.rcdt_ns import RCDT_NS
from utils import *
        ...
        for repeat in range(num_repeats):
            ...
            if args.classifier == 'subspace':
                ...
                if args.count_flops:
                    ...
                    high.start_counters([events.PAPI_DP_OPS,])
                    if args.use_image_feature:
                        preds = classifier.predict(x_test,datanm=args.dataset)
                    else:
                        preds = classifier.predict(x_test, args.use_gpu,datanm=args.dataset)
                ...
                preds = classifier.predict(x_test_rcdt_flat,datanm=args.dataset)
            toc = time.time()
